...
* Lyft fram 3-5 saker som varit svåra:
    * Random-rum
    * Trait tillsammans med Entity-klass
    * Tester - väldigt mycket mockande, framför allt pga random-elementen
...
* Lägg och ta bort rum via ett enkelt api, spelet använder de rum som finns för att generera labyrinten.

För dig som rättar finns en "show hint"-knapp som visar hur du löser questen på snabbaste sätt.

Under huven använder spelet MVC-mönstret som varit fokus under kursen, med så lite logik som möjligt i kontrollern och vyerna. Jag började med att göra ett ganska detaljerat flödesschema 

**(flödesschemat)**

Krav 4
(kan inte klicka på alla länkar)
