...
Under huven använder spelet MVC-mönstret som varit fokus under kursen, med så lite logik som möjligt i kontrollern och vyerna. Jag började med att göra ett enkelt övergripande flödesschema och sedan var det bara att implementera:
...
Ingången för att hämta alla rum levererar en json med all data om rummen, inklusive bilden som också lagras i databasen. Det innebär att om inte t ex webbläsaren eller postman hjälper till att snygga till outputen så blir svaret väldigt långt. Jag övervägde om jag bara skulle skicka med information om att det finns bild men jag bedömde att ingången är till för att faktiskt kunna hämta all data om rummet. 
...
Man kan nog säga att slumpmässigheten i stort tillsammans med att innnehållet (rum och items) inte är hårdkodat är det som sticker ut mest med projektet, och som kanske varit svårast att få till och samspela på ett bra sätt. Rummen placeras i ett rutnät slumpmässigt, dörrarna placeras ut slumpmässigt så det bildas en labyrint, items placeras ut slumpmässigt i rummen och utifrån det byggs quester ihop med en item och ett slumpmässigt rum som mål. 
